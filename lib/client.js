 ...
 *   (rcmd API, proxied through the host) rendered as a card grid — browse it
 *   like the homepage, no login needed.
 * - Click a card → its BV id is used to jump the embedded Bilibili video
 *   page inside the window (own account, danmaku, HD). The iframe is
 *   sandboxed without popup / top-navigation permissions, so nothing can
 *   open a new tab or leave the small window.
 *
 ...
 * (unmounted to silence audio) and a semi-transparent mask appears prompting
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const RCMD =
      "https://api.bilibili.com/x/web-interface/index/top/rcmd?fresh_type=3&ps=30&fresh_idx=";

    function BiliWatch(props) {
      const useSessions = props.useSessions;
      const [open, setOpen] = React.useState(true);
      const [view, setView] = React.useState("feed"); // 'feed' | 'video'
      ...
      if (view === "video" && videoUrl) {
        // Unmounted while attention: stops audio. sandbox: no popups / top nav.
