 ...
 *
 * Behavior: a floating bottom-right Bilibili mini-player for the DeepSeek
 * conversation. A home feed (Bilibili ranking) lets you pick videos like a
 * normal homepage; playback uses a native <video> fed by the host's stream
 * proxy (/dsh-bili/stream), so the player can pause and resume at the exact
 * position. When the agent needs you (approval / question / plan review) or
 * stops (completed / blocked / errored) the video pauses and a reminder is
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    const CSS = `
      ...
      .bw-panel { position: fixed; right: 18px; bottom: 18px; width: 420px; pointer-events: auto; background: rgba(24,26,32,.97); color: #fff; border: 1px solid rgba(255,255,255,.14); border-radius: 14px; box-shadow: 0 16px 48px rgba(0,0,0,.55); z-index: 10001; overflow: hidden; backdrop-filter: blur(10px); display: flex; flex-direction: column; }
    ...
    function BiliWatch(props) {
      ...
      const loadVideo = React.useCallback(async (bvid, page) => {
        try {
          ...
          setVideo({
            ...
            src: "/dsh-bili/stream?u=" + encodeURIComponent(du.url),
          });
          setTab("player");
          setAttention(null);
      ...
      if (attention) {
        ...
        const stopped = video
          ? el("div", { className: "bw-stopped" },
              ...
              el("div", { className: "bw-stopped-hint" }, "已记住播放进度，点「继续看视频」从当前位置续播"),
            )
          : null;
      ...
      const panel = el("div", { className: open ? "bw-panel" : "bw-panel bw-panel-hidden" },
        header,
