 ...
 * account, danmaku, HD playback. The iframe is sandboxed without popup /
 * top-navigation permissions, so nothing can ever open a new tab or leave
 * the small window; the entry page is the popular ranking feed whose video
 * cards navigate in-place (Bilibili's own homepage feed is hardcoded to
 * open new tabs and is therefore blocked by the sandbox).
 *
    ...
    function BiliWatch(props) {
      ...
      const page = attention
        ...
        : el("div", { className: "bw-site-zoom", style: { "--bw-zoom": String(zoom) } },
            el("iframe", { key: siteUrl, className: "bw-site-frame", src: siteUrl, title: "Bilibili", sandbox: "allow-scripts allow-same-origin allow-forms allow-modals allow-downloads" }),
          );

      const content = el("div", { className: "bw-site-wrap" },
        page,
        el("div", { className: "bw-site-hint" },
          el("span", null, "全程小窗内完成：点视频 / 相关推荐不跳转。B站首页与搜索的「新标签打开」已被浏览器安全限制拦截（点了不跳转），请用热门榜、相关推荐或下方输入框打开视频。"),
      ...
      const bottom = el("div", { className: "bw-open" },
        ...
        el("button", { className: "bw-btn", onClick: () => setSiteUrl("https://www.bilibili.com/v/popular/rank/all") }, "热门"),
      );
