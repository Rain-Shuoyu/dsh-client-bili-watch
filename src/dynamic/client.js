return {
  ...
  apply(ctx) {
    ...
    async function biliApi(url) {
      const res = await host.call('bili-api', { url: String(url) })
      return res
    }
    ...
    function BiliWatch(props) {
      ...
      const loadFeed = React.useCallback(() => {
        ...
        setFeed(null)
        setFeedError('')
        biliApi('https://api.bilibili.com/x/web-interface/ranking/v2?rid=0&type=all').then((res) => {
          if (!alive) return
      ...
      const loadVideo = React.useCallback(async (bvid, page) => {
        try {
          ...
          const pl = await biliApi('https://api.bilibili.com/x/player/playurl?bvid=' + bvid + '&cid=' + cid + '&qn=64&fnval=0')
      ...
      if (attention) {
        ...
        const msg = attention.message ? el('div', { className: 'bw-msg' }, String(attention.message)) : null
        const stopped = video ? el('div', { className: 'bw-stopped' },
